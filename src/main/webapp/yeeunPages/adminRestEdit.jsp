...
<div class="container">
	...
	<div class="wrapper-yeeun">
		...
		<form id="rest-insert-form" method="post" action="">
			<fieldset>
				...
				<div class="mg30">
					...
					<textarea id="rest_desc">${list.get(0).rest_desc }</textarea>
				</div>
			</fieldset>
			<fieldset>
				<legend>영업시간</legend>
				...
				<table class="mg70 mg30">
					...
					<tbody>
						...
						<tr>
							<td>일,공휴일</td>
							<td><input class="input-block" type="time"
								title="일,공휴일 오픈시간 입력" name="biz_open" id="biz_open_7"
								value="${list.get(1).get(6).biz_open}"
								<c:if test="${list.get(1).get(6).biz_off eq 1}">readonly</c:if> />
							</td>
							<td><input class="input-block" type="time"
								title="일,공휴일 마감시간 입력" name="biz_close" id="biz_close_7"
		...
		<div class="btn-box">
			<!-- 저장버튼. 목록버튼-->
			<a href="adminRestDelete.ye?rest_no=${list.get(0).rest_no}" class="red-short-btn">삭제</a>
